...
def getXAuth():
    ...
    return hashlib.sha256(f"{config['username']}{config['password']}aa76fbc1f1440caf93423c42e6c3bb5a".encode("utf-8")).hexdigest()

def getMachineId():
    with open('/worker_id.cfg') as f:
        output = f.read()
        return output

global_WUText = None
global_XAuth = getXAuth()
global_machineId = getMachineId()
...
def run_command(command, stdin=None, print_error=True, **kwargs):
    ...
    start = time.time()
    child = subprocess.Popen(command_list, env=dict(os.environ, LD_PRELOAD="/shim_main.so"),
    ...
    if print_error and child.returncode != 0:
        ...
        logging.info("%s finished after %s seconds", command_list[0], str(round(time.time() - start, 2)))
...
class HTTP_connector(object):
    ...
    def _urlopen_maybe_https(request, cafile=None, check_hostname=True):
        ...
        """
        global global_XAuth
        global global_machineId
        if not isinstance(request, urllib_request.Request):
            request = urllib_request.Request(request)
        request.add_header("X-AUTH", global_XAuth)
        request.add_header("X-MACHINE", global_machineId)
    ...
    def _native_get_file(self, url, dlpath, cafile=None, wait=None):
        global global_WUText
        isWU = "/cgi-bin/getwu" in url
        ...
        if isWU:
            global_WUText = request.read().decode("utf-8")
        else:
...
class WorkunitWrapper(Workunit):
    ...
    def __init__(self, filename, peer):
        ...
        Workunit.__init__(self, global_WUText)
        return
...
class InputDownloader(object):
    ...
    def get_file(self, urlpath,
        ...
        if not is_wu and dlpath_tmp is not None:
    ...
    def get_missing_file(self, urlpath, filename,
        ...
        if filename.endswith(".roots1.gz") or filename.endswith(".poly"):
            waitTimeMax = round(20 + 15 * random.random(), 2)
            logging.info("Waiting for fact_worker to create %s (up to %s seconds)", filename, str(waitTimeMax))

            waited = 0
            while waited < waitTimeMax and not os.path.isfile(filename):
                time.sleep(0.01)
                waited += 0.01

            if os.path.isfile(filename):
                logging.info("Found %s, proceeding", filename)
                return None
            else:
                logging.warning("%s still missing after %s seconds. Falling back to downloading", filename, str(waitTimeMax))
    ...
    def process_uploads(self):
        ...
        for i in range(self.nservers):
            ...
            for p in old_backlog:
                ...
                conn.close()
                did_progress = True
                logging.info("Upload of %s succeeded after %s seconds.", p.workunit.get_id(), str(round(time.time() - start, 3)))
                p.cleanup()
